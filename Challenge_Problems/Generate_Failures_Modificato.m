...
effector_type       = ones(num_traj,1); % GUASTO SOLO ALLE SUPERFICI   
% ----------------
...
surf_fail_ind       = 5*ones(num_traj,1); % GUASTO SOLO ALLA SUPERFICE 5
surf_fail_ind       = ones(num_traj,1); % GUASTO SOLO ALLA SUPERFICE 1
% ----------------
...
for traj_num = 1:num_traj
    ...
    % --- MODIFICA ---
    if fail_type(traj_num) >=0 && fail_type(traj_num) < 0.25
            fail_type_act = 1; % Hold last failure
    elseif fail_type(traj_num) >= 0.25 && fail_type(traj_num) < 0.50
            fail_type_act = 2; % Pre-scale failure, (failure in command prior to actuator dynamics)
    elseif fail_type(traj_num) >= 0.50 && fail_type(traj_num) < 0.75
            fail_type_act = 3; % Post-scale failure, (failure after actuator dynamics)
    elseif fail_type(traj_num) >= 0.75 && fail_type(traj_num) <= 1.0
